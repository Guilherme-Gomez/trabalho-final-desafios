...
#Simulando com menos dados
#createOrReplaceTempView(where(df,df$CODIGO_VAGAO=='98161'), "tabela_pior_trem")
...
# tabela_join %>% drop(c("ROW_ID","DATA_HORA_LEITURA","CODIGO_VAGAO","CODIGO_RODEIRO","LADO_RODA","EIXO_VAGAO","CICLO_RODEIRO","SENTIDO_TREM","TRAIN_ID","POSICAO_VAGAO_COMPOSICAO")) %>% #head(100) %>% 
#   collect() %>% 
#   View()

#Foi necessário criar um rank pois as rodas tem medidas em dias diferentes.
library(stringr)
df_experiencia <- tabela_join %>% drop(c("ROW_ID","DATA_HORA_LEITURA","CODIGO_VAGAO","CODIGO_RODEIRO","LADO_RODA","EIXO_VAGAO","CICLO_RODEIRO","SENTIDO_TREM","TRAIN_ID","POSICAO_VAGAO_COMPOSICAO","DIA")) %>% 
#definindo o perído avaliação.
  where(.,.$Rank<10) %>% 
  collect() %>% 
#usando o str_pad para conseguir organizar melhor as colunas.
  dplyr::mutate(Rank_2=str_pad(.$Rank, 2, pad = "0")) %>% 
  dplyr::select(-Rank) %>%
  pivot_wider(names_from = c(Rank_2),values_from = c(colunas_numeric$medidas,"FRISO_MENOR_QUE_26")) %>%
  dplyr::select(sort(names(.))) %>%
  dplyr::select(CODIGO_RODA,FRISO_MENOR_QUE_26_01,everything()) %>% 
#Removendo as colunas das medições atuais e mantendo a variável dependente.
  dplyr::rename(FRISO_MENOR_QUE_26=FRISO_MENOR_QUE_26_01) %>% 
  dplyr::select(-colnames(.[which(substr(colnames(.),nchar(colnames(.))-1,nchar(colnames(.)))=="01")]))

head(df_experiencia)
class(df_experiencia)
```
...
```{r Area_de_teste_Undersampling, cache=TRUE}
...
df_train_teste <- df_undersampling
df_test_teste <- except(df_clean, df_undersampling)
...
print(paste("A base de treino ficou com ",
            dim(df_train_teste)[1],
            " o que equivale  ",
            round((dim(df_train_teste)[1]*100/dim(df_clean)[1]),2)," % da base limpa",sep=""))
...
print(paste("A base de testes ficou com ",
            dim(df_test_teste)[1],
            " o que equivale a ",
            round((dim(df_test_teste)[1]*100/dim(df_clean)[1]),2)," % da base limpa",sep=""))

# Modelo Reg Logistica

model_teste_rl <- spark.logit(df_train_teste, FRISO_BAIXO ~ 
                       VELOCIDADE_ENTRADA_TREM+
                       VELOCIDADE_SAIDA_TREM+
                       ANGULO_FRISO_RODA+
                       ALTURA_FRISO_RODA+
...
summary(model_teste_rl)

fitted_teste_rl <- predict(model_teste_rl, df_test_teste)
```

```{r Area_de_teste_Undersampling_avaliacao}
# Avaliacao

createOrReplaceTempView(fitted_teste_rl, "predictions_teste")
...
```{r Area_de_teste_Undersampling_RF}
# Area para realizacao de testes
# colunas_numeric
#colnames(df_clean)
...
model_teste_rf <- spark.randomForest(df_train_teste, FRISO_BAIXO ~ 
                       VELOCIDADE_ENTRADA_TREM+
                       VELOCIDADE_SAIDA_TREM+
                       ANGULO_FRISO_RODA+
                       ALTURA_FRISO_RODA+
...
summary(model_teste_rf)

fitted_teste_rf <- predict(model_teste_rf, df_test_teste)

```

```{r Area_de_teste_Undersampling_RF_avaliacao}
# Avaliacao

createOrReplaceTempView(fitted_teste_rf, "predictions_teste")
