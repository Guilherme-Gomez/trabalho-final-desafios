...
**Problema:** Prever se o friso da roda está abaixo de 26 mm na próxima leitura.
...
Verificamos a concentração de dados nulos nas variáveis CAVA_RODA, ANGULO_FRISO_RODA e nas geradas pelo TBOGIE.
...
Avaliando a ocorrência de dados nulos de CAVA_RODA e ANGULO_FRiSO_RODA 

CAVA_RODA: A variável com maior número de dados nulos foi a CAVA_RODA com 607.049 (20%) de dados nulos. 
...
Como podemos perceber a ocorrência de dados nulos é distribuída por todo o período de medição sendo que ocorreu uma acentuação nos erros no segundo semestre de 2018. Essa acentuação é proporcional ao aumento do número de dados da amostra que aumentou significativamente no período. 

```{r Avaliacao_CAVA_RODA_ANGULO_FRISO, message=FALSE, warning=FALSE}

# Avaliando os dados nulos para CAVA_RODA, identificando se ocorreu alguma data com maior numero de dados nulos.
...
Avaliando as variáveis de TBOGIE  
<br>  
ANGULO_ATAQUE_EIXO_FRONTAL_TRUQUE, ANGULO_ATAQUE_EIXO_TRASEIRO_TRUQUE, TRACKING_POSITION_EIXO_FRONTAL_TRUQUE.

Essas variáveis apresentam 19.723 de valores nulos, o que equivale a 1% do total dos dados.
...
Total de dados da base original: `r as.character(round(nrow(df)[1],0))` 
...
Total de dados da base limpa: `r as.character(round(nrow(df_clean)[1],0))` 

```{r Limpeza_dados_2, include=FALSE}
# Redução dos dados
reduc <- round(((dim(df_clean)[1]/dim(df)[1])-1)*100,2) 
reduc <- paste("Redução dos dados foi de ",-reduc,"%",sep="")
```
...
Também concluimos que todas as variáveis possuem valor máximo e mínimo dentro dos limites esperados.
...
Em relação as variáveis numéricas, consideramos interessante analisar se existiam correlações relevantes.

É possível perceber que algumas variáveis possuem correlação:

| Variável | Variável | 	Correlação |
|------------|---------|---------:|
| VELOCIDADE_ENTRADA_TREM (VEL_ET)|   VELOCIDADE_SAIDA_TREM (VEL_ST) | (0.91)
| ANGULO_ATAQUE_EIXO_TRASEIRO_TRUQUE (ANG_AETT)| ALINHAMENTO_ENTRE_EIXOS_(IAM)(ALI_EE)  | (0.76)
| TRACKING_POSITION_EIXO_FRONTAL_TRUQUE (TR_PEFT) | DESLOCAMENTO_ENTRE_EIXOS_(SHIFT)(DES_EE)  | (0.72)

Abreviações utilizadas nos gráficos
...
ggheatmap + 
  labs(title = "Mapa de correlação das variáveis")+
...
* É possível notar um grande aumento no volume de dados medidos após o mês de Maio de 2018.  

* O tamanho das composições aumenta na mesma data.
...
Utilizamos 2 abordagens para realizar a preparação dos dados.
...
A base df_clean é a base limpa com o balanceamento da base de testes.
O balanceamento (undersample) foi realizado da seguinte forma: 

* Selecionados todos os dados da base com FRISO_BAIXO = 1 
* Selecionados de forma aleatória (Sample) a quantidade igual a 3 vezes o número de linhas da base anterior com FRISO_BAIXO = 0.
* Combinação das duas bases, criando a base de treino.
* Base de testes, sendo a base total menos os dados da base de teste.
...
1.  Under sampling: Para treino foram utilizadas as medições das rodas que estiveram no trem com o maior número de rodas com friso baixo na última medição. Por exemplo: TRAIN_ID = 25B7F2F33 com 157 rodas com FRISO_BAIXO=1.


```{r echo=FALSE, message=FALSE, warning=FALSE}

createOrReplaceTempView(where(df,df$DIA >='2018-08-19' ), "tabela_ultimo_ciclo")

tabela_pior_trem <- sql("select TRAIN_ID,DIA, count(*) Quantidade_Rodas_Friso_Baixo

              from tabela_ultimo_ciclo 
              where FRISO_BAIXO=1
              group by TRAIN_ID,DIA
              order by Quantidade_Rodas_Friso_Baixo desc
")
piores_trens <- tabela_pior_trem %>% head(5)

tabela_pior_trem %>% head(5) %>% kable(., format = "html",row.names = T, digits = 2) %>%
  kable_styling(bootstrap_options = c("striped","hover","condensed","responsive"))
```

2.  Com tabelas auxiliares foi incluído na base histórica a ordem da medição (Rank).
3.  Foram removidas as colunas que não foram utilizadas no modelo, ex.: DATA_HORA_LEITURA.
4.  Filtrada a quantidade de versões (Rank) de medições que entraram no modelo (RanK<=2 filtra apenas as medições da versão anterior e da atual).
...
9.  O mesmo processo é realizado para a base de teste, mas apenas com dados que não estão no(s) trem(ns) utilizado(s) para treino.
...
#createOrReplaceTempView(where(df,df$TRAIN_ID %in% piores_trens$TRAIN_ID), "tabela_pior_trem")
...
head(df_pivot_spark) %>% rmarkdown::paged_table()
...
### Modelo 1 {.tabset .tabset-fade .tabset-pills}

O primeiro modelo criado foi utilizando a base clean e não foram encontrados bons resultados.
...
### Modelo 2 {.tabset .tabset-fade .tabset-pills}

Como o modelo 1 não teve uma boa performance, decidimos buscar um tratamento da base para incluir os dados históricos de medição (df_pivot). Com isso, a regressão logística não apresentou um bom desempenho porém o modelo random forest apresentou um resultado satisfatório, sendo este o escolhido para a solução do problema. 
...
Neste trabalho foi possível entender as reais necessidades que os projetos com grande volume de dados demandam e criar com sucesso um modelo para prever o comportamento das variáveis apresentadas no problema.  
Toda a organização da equipe e os processos desenvolvidos ao longo do trabalho foram baseadas na metodologia CRISP-DM.   
Para viabilizar as contribuições da equipe foi primordial a utilização de versionamento na plataforma do github.   
O trabalho foi realizado utilizando as técnicas de *map reduce* implementadas no *spark* que possibilitaram o processamento paralelo e utilização controlada dos recursos disponíveis. Por vezes, as máquinas pessoais chegaram ao limite de seus recursos demandando a utilização de ferramentas em nuvem tanto com  saas (máquinas virtuais) como através da utilização do paas (DataBricks), mas com o aprimoramento dos conhecimentos da equipe as próprias máquinas pessoais passaram a desempenhar de maneira satisfatória.
A utilização das técnicas aprendidas no curso para visualizar os dados permitiu debates sobre a natureza do problema envolvendo a rotina de operações ferroviárias, novo para boa parte da equipe. Foi possível entender as questões envolvidas nos desgastes das peças aplicar as técnicas aprendidas ao longo do curso.
Ao final do trabalho, após extensa experimentação, conseguimos encontrar um modelo de Random Forest mais adequado que apresentou 1627 de previsões varedadeiro-positivo o que representa uma precisão de 66%. 
