...
**FRISO_BAIXO**: Indica com 1 ou 0 se a ESPESSURA_FRISO_RODA é maior ou menor que 26 mm. Utilizada como varivável DEPENDENTE nos modelos.  
<br>
...
Verificamos a concentração de dados nulos nas variaveis CAVA_RODA, ANGULO_FRISO_RODA e nas variaveis do TBOGIE
...
Como podemos perceber a ocorrência de dados nulos é distribuido por todo o periodo de medição sendo que ocorreu uma acentuação nos erros no segundo semestre de 2018. Essa acentuação é proporcional ao aumento do número de dados da amostra que aumentou significativamente no período. 
...
Em relação as variaveis provenientes da medição TBOGIE todas apresentaram um comportamento igual, ou seja, todas os casos nulos ocorreram no mesmo periodo, como todos essas dados são do T-BOGIE, acreditamos que ocorreu uma falha nesse sistema de medição nesses períodos.
...
Total de dados da base original: `r nrow(df)[1]` 
...
Total de dados da base limpa: `r nrow(df_clean)[1]` 
...
### Identificar as principais estatisticas da Amostra {.tabset .tabset-fade .tabset-pills}
...
Utilizamos 3 abordagens para realizar a preparação dos dados (ordenada na ordem de complexidade)
  ...
  + Utilizar a base limpa (df_clean) sem alteração nos registros (df_clean)
...
(descrição do procedimento do pivot)

Criando a base de treino df_pivot_spark (outro trem)
...
#createOrReplaceTempView(where(df_clean,df_clean$DIA>='2018-06-18'), "tabela_pior_trem")
