...
sdf_numeric <- select(df_clean,
                     ...
                     "TRACKING_ERROR_(TE)") %>% 
                       sample(.,withReplacement=FALSE,fraction = 0.01)%>% collect(.)


colnames(sdf_numeric) <- c("VEL_ET",
                           "VEL_ST",
                           "ANG_FR",
                           "ANG_FR",
                           "ESP_FR",
...
chart.Correlation(sdf_numeric, histogram=TRUE, pch=19)

#cor(sdf_numeric)

cormat <- round(cor(sdf_numeric),2)
#cormat
...
```{r Guilherme 6, message=FALSE, warning=FALSE}
...
```{r Guilherme 7, message=FALSE, warning=FALSE}
...
df_list <- randomSplit(sdf_clean, c(3,7),2)
...
model_rl <- spark.logit(df_train, FRISO_MENOR_QUE_26 ~ 
                       VELOCIDADE_ENTRADA_TREM+
                       VELOCIDADE_SAIDA_TREM+
                       VELOCIDADE_ENTRADA_TREM+
...
```{r Davi 13, cache=TRUE}
# benchmarking

colnames(df_clean)
bench <-summarize(groupBy(df_clean, 
                          df_clean$FRISO_MENOR_QUE_26), 
                          count = n(df_clean$ROW_ID))
...
createOrReplaceTempView(fitted_rf, "predictions")
correct <- sql("SELECT prediction, FRISO_MENOR_QUE_26 FROM predictions WHERE prediction=FRISO_MENOR_QUE_26")
acc_rf = count(correct)/count(fitted_rf)
acc_rf
