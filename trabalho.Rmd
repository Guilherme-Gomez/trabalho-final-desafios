...
Há valores nulos nos campos dos resultados das medições. É notável que há erros em algumas linhaspara todas medições do T-BOGIE.  
...
#Funções que serão utilizadas dentro do loop para varrer o dataset e contabilizar os problemas.
#Foi necessário utilizar o try, pois alguma funções retornam erro para variáveis do tipo date ou datetime.
...
#Dataframe para contabilizar os erros dentro do loop.
...
df_sample %>% where(.,isNull(.[["ANGULO_FRISO_RODA"]]))%>% head(100) %>% View()
df_sample %>% where(.,!isNull(.[["ANGULO_FRISO_RODA"]]))%>% head(100) %>% View()
...
df2 %>% head(1000) %>% View()
#Algumas rodas nunca tiveram medidas de ângulo (20 medidas com NA)
...
#Exemplo de rodas com falta de dados e opções de completar com fill (dados de outras medições)
...
df_sample %>% where(.,isNull(.[["ALTURA_FRISO_RODA"]]))%>% head(100) %>% View()
df_sample %>% where(.,isNull(.[["ESPESSURA_FRISO_RODA"]]))%>% head(100) %>% View()
df_sample %>% where(.,isNull(.[["CAVA_RODA"]]))%>% head(100) %>% View()
